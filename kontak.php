            ...
            <div class="card-body">
              ...
              <a href="tambah.php">Tambah</a>
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                  ...
                  <?php
                    $qrcdu=$_POST["qruser"];
                    $waktuu=$_POST["waktuuser"];
                    //echo $waktuu;
                    $btsdays    =strtotime (' - 2 hours', strtotime($waktuu));
                    $tglakhir = date('Y-m-d h:i:s', $btsdays);
                    $tempatu=$_POST["tempatuser"];
                    $d=strtotime("-2 Hours");
                    $d1=date($waktuu, strtotime("-2 Hours"));
                   // echo $tglakhir;
                  ?>
                  <?php
                    $no = 1;
                    $query = "SELECT tbl_user.nama, 
                    tbl_tempat.nama_tempat, 
                    tbl_petugas.nama_petugas, 
                    hasil_scan.tanggal, 
                    hasil_scan.qrcode 
                    FROM hasil_scan JOIN tbl_user ON hasil_scan.qrcode=tbl_user.username 
                    JOIN tbl_tempat ON hasil_scan.id_tempat=tbl_tempat.id_tempat 
                    JOIN tbl_petugas ON hasil_scan.id_petugas=tbl_petugas.id_petugas
                    WHERE hasil_scan.tanggal >= ' $tglakhir' AND tanggal <= '$waktuu'";
